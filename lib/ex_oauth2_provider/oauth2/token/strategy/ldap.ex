defmodule ExOauth2Provider.Token.Ldap do
  @moduledoc """
  ...
  def grant(%{"grant_type" => "ldap"} = request, config \\ []) do
    {:ok, %{request: request}}
  ...
  defp get_password_auth_method({:ok, params}, config) do
    case Config.ldap_auth(config) do
      {module, method} -> {:ok, Map.put(params, :ldap_auth, {module, method})}
